...
export const metadata: Metadata = {
  ...
  description: "Complete guide to Plus One and Plus Two admissions in Kerala. HSCAP, school allotment, documents required, and how to apply for HSE admission near Vatanappally.",
};
...
const sections = [
  ...
  { title: "HSCAP Timeline", items: ["May: SSLC results announced", "May–June: HSCAP registration opens online", "June: First allotment published on portal", "June–July: Supplementary allotments for remaining seats", "June–July: Unaided school admissions (separate process)", "July: Classes begin for Plus One"], accent: 'var(--blue)' },
                ...
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {section.items!.map(item => (
                    <li key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '13px', color: 'var(--muted)', lineHeight: 1.65 }}>
                      <span style={{ color: 'var(--lime-2)', fontWeight: 800, flexShrink: 0, marginTop: '1px' }}>→</span>
                      {item}
                    </li>
                  ))}
                </ul>
